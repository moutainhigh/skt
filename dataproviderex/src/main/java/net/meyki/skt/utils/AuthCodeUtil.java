...
public class AuthCodeUtil {
	...
	public static String CutString(String str, int startIndex, int length) {
		...
		String ret= str.substring(startIndex, startIndex + length);
		System.out.print("\ncutstring="+ret);
		return ret;
	}
	...
	private static String authcode(String source, String key,
		...
		try {
			...
			if (operation == DiscuzAuthcodeMode.Decode) {
				...
				temp = BaseCode64.decode(CutString(source, ckey_length));
				result = new String(RC4(temp, cryptkey));
				Log.v("CutString(result, 10, 16)", CutString(result, 10, 16));
				Log.v("CutString(MD52(CutString(result, 26) + keyb), 0, 16)", CutString(MD52(CutString(result, 26) + keyb), 0, 16));
				if (CutString(result, 10, 16).equals(
					...
					if (CutString(result, 10, 16)
						...
						temp = BaseCode64.decode(CutString(source + "==",
								ckey_length));
						result = new String(RC4(temp, cryptkey));
						if (CutString(result, 10, 16).equals(
				...
				return keyc + BaseCode64.encode(temp);
	...
	public static void main(String[] args) {
		...
		String deStr1 = AuthCodeUtil.authcodeDecode(
				"HeGUeCVbBYBdVwK9aSGwi0b1B2sfdClZSpjR", "czyb");
